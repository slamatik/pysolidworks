...
class IFeatureManager:
    ...
    def group_component_instances(self):
        """Gets or sets whether to group the same components in the same configuration in an assembly into a folder in
    ...
    def show_component_configuration_descriptions(self):
        """Gets or sets whether to show the active configuration's component configuration descriptions in the
    ...
    def show_component_configuration_names(self):
        """Gets or sets whether to show the active configuration's component configuration names in the FeatureManager
        design tree."""
    ...
    def add_corner_relief_corner(self):
        """Adds the bend corner of two selected faces of a sheet metal body to the set of corners to which to apply a
        corner relief."""
    ...
    def draft_xpert_change(self):
        """Changes the parameters on the selected drafted faces, regardless of whether the drafted faces were created
        manually or with DraftXpert, provided that DraftXpert can process them."""
    ...
    def draft_xpert_remove(self):
        """Deletes the draft on the selected faces. If all the faces of a draft are selected, then this method deletes
        the draft feature; if not, then this method edits the draft feature and removes the selected face references
        from it."""
    ...
    def fillet_xpert_change(self):
        """Changes the parameters on the selected filleted faces, regardless of whether the filleted faces were created
        manually or with FilletXpert, provided that FilletXpert can process them."""
    ...
    def get_pre_trimmed_bodies(self):
        """Gets the temporary trimmed bodies using the specified target sheet (surface) body according to the trim tools
        previously defined by IFeatureManager::PreTrimSurface."""
    ...
    def i_insert_reference_point(self):
        """Creates the geometry for the reference points based on any of these selected entities: edges, faces, planes,
        vertices, or sketch geometry."""
    ...
    def insert_sub_folder(self):
        """Creates a subfolder in the Solid Bodies folder in the FeatureManager design tree and moves the selected solid
        bodies or subfolders in the Solid Bodies folder to the new subfolder."""
    ...
    def insert_untrim_surface(self):
        """Inserts an untrimmed surface to patch surface holes and external edges by extending an existing surface along
    ...
    def move_to_folder(self):
        """Moves the selected feature or folder in the Solid Bodies Feature Manager design tree structure to the
        specified folder in the Solid Bodies Feature Manager design tree structure."""
    ...
    def set_net_blend_direction_data(self):
        """Sets the curve set data (one for each of the two directions) for this boundary feature or boundary surface
        feature."""
